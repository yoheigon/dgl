    ...
    def forward(self, G, h):
        with G.local_scope():
            ...
            for srctype, etype, dsttype in G.canonical_etypes:
                ...
                k = k_linear(h_src).view(-1, self.n_heads, self.d_k)
                v = v_linear(h_src).view(-1, self.n_heads, self.d_k)
                q = q_linear(h_dst).view(-1, self.n_heads, self.d_k)

                e_id = self.edge_dict[etype]

                relation_att = self.relation_att[e_id]
                relation_pri = self.relation_pri[e_id]  #prior tensor
                relation_msg = self.relation_msg[e_id]
                ...
                sub_graph.srcnodes[srctype].data["k"] = k
                sub_graph.dstnodes[dsttype].data["q"] = q
                sub_graph.srcnodes[srctype].data["v_%d" % e_id] = v
    ...
    def forward(self, G, out_key):
        ...
        for i in range(self.n_layers):
            ...
            for ntype in block.ntypes:
                ...
                h_src = F.gelu(self.adapt_ws[n_id](block.srcnodes[ntype].data["inp"])) 
                h_dst = F.gelu(self.adapt_ws[n_id](block.dstnodes[ntype].data["inp"])) 
                h[ntype] = (h_src, h_dst)
            h = self.gcs[i](block, h)
